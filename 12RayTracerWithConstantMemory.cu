...
sphereStruct *SpheresCPU; //*SpheresGPU;                                                       //remove pointer to global memory
__constant__ sphereStruct ConstantSpheres[NUMSPHERES];                                         //move to constant memory

cudaEvent_t start, stop;                                                                       //make global variables for the timing
...
{	
	if(key == 'q')
	{
		cudaEventDestroy(start);                                                             //add clean up for timing events
...
__global__ void makeSphersBitMap(float *pixels/*, sphereStruct *sphereInfo*/)                       //remove unneeded global pointers
{
	...
	{
		hitValue = hit(pixelx, pixely, &dimingValue, /*sphereInfo[i]*/ ConstantSpheres[i]);        //remove global sphere info and replace with constant
		...
		if(maxHit < hitValue)
		{
			...
			pixelr = ConstantSpheres[i].r * dimingValue; 	                                                                     //now use contant info
			pixelg = ConstantSpheres[i].g * dimingValue;	
			pixelb = ConstantSpheres[i].b * dimingValue; 	
...
{	
	...
	//move this section over from makeBitMap to only copy memory once
	cudaMemcpyToSymbol(ConstantSpheres, SpheresCPU, NUMSPHERES*sizeof(sphereStruct)/*, cudaMemcpyHostToDevice*/);  //use new constant sphere info
	cudaErrorCheck(__FILE__, __LINE__);
}	

void makeBitMap()
{	         
	cudaEventRecord(start);                                                        //mark start event here      
	                                             
	//move this section to makeRandomSpheres to only copy once                     //use cudaMemcpyToSymbol when using constant instead of global memory
	//cudaMemcpyToSymbol(ConstantSpheres, SpheresCPU, NUMSPHERES*sizeof(sphereStruct)/*, cudaMemcpyHostToDevice*/);  //use new constant sphere info
	//cudaErrorCheck(__FILE__, __LINE__);
	
	makeSphersBitMap<<<GridSize, BlockSize>>>(PixelsGPU/*, SpheresGPU*/);          //remove unneeded global pointer
	cudaErrorCheck(__FILE__, __LINE__);

	cudaEventRecord(stop);                                                         //mark stop event here
	cudaEventSynchronize(stop);
	
	float milliseconds = 0;                                                        //make variable to store time passed
	cudaEventElapsedTime(&milliseconds, start, stop);                              //change milliseconds to be the elapsed time
	cudaErrorCheck(__FILE__, __LINE__);                                            //error check
	printf("Kernel execution time was %f milliseconds\n", milliseconds);           //print time
...
void setup()
{
	...
	//*cudaMalloc(&SpheresGPU, NUMSPHERES*sizeof(sphereStruct));*//                                               //moved to constant memory
	//cudaErrorCheck(__FILE__, __LINE__);

	cudaEventCreate(&start);                                                                                      //make timing "events", marks the start and stop
	cudaErrorCheck(__FILE__, __LINE__);
