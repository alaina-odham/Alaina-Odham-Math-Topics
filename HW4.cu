...
// Defines
#define N 9999                                                                                   // Changed length of the vector
...
{
	for(int id = 0; id < n; id++)                              //changed this formula to be computationally expensive. Took CPU around 587 and GPU 6776 microseconds when tested.
	{ 
...
{                                                                     //We want to edit the GPU process to be able to compute any vector length.
	int id = blockIdx.x*blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;                              //define stride so it can be used
	
	#pragma unroll                                                    //Pragma unroll takes a loop and does each loop simultaneously and seperately if possible to save time.
	for(int i = id; i < n; i += stride)                               //Add the stride to thread ID so we can loop
	{
		if (i < n)                                                    //make sure we do not use unalloted memory
        {
            c[i] = a[i] + b[i];                                       //After adding the pragma unroll took CPU around 596 and GPU 142 microseconds when tested. Big improvement! :D
        }
	}
}
