...
void drawPicture()
{
	...
	cudaMemPrefetchAsync(P, N*sizeof(float3), cudaCpuDeviceId);                        //use unified memory to fetch data for all N positions, use in following for loop
	cudaErrorCheck(__FILE__, __LINE__);
...
void setup()
{
	...
	GridSize.z = 1;
		...
		*/
		...
		cudaMallocManaged(&M, N*sizeof(float));                                                                         //use this instead to allocate unified memory for N floats
		cudaErrorCheck(__FILE__, __LINE__);
    	...
    	// Allocating the first pointers that point to M, P, V, and F of the GPUs but actually reside on the CPU       //remove old memory setup
	...
	//for(int i = 0; i < NumberOfGpus; i++)                                                   //remove this old memory method
    //{
	//	cudaSetDevice(i);
	...
	for(int i = 0; i < NumberOfGpus; i++)                                                    //prefetch data at end up setup before simulation runs, run thorugh all gpus (needs forces from all bodies)
	{
   		cudaSetDevice(i);
		cudaErrorCheck(__FILE__, __LINE__);
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		...
		for(int i = 0; i < NumberOfGpus; i++)
    	{
			//cudaSetDevice(i);                                                                                      //remove old method
			...
			cudaSetDevice(i);                                                                                        //use unified memory pointers instead
		...
		// Copying memory between GPUs.                                                                              //remove old method
		//for(int i = 0; i < NumberOfGpus; i++)
    	//{
		//	cudaSetDevice(i);
