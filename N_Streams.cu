...
//notes:
//a cuda stream is a chain of stuff to do in cuda
//can have more than one string at once, so gpu and cpu can run simultaneously
//data transfer between gpu and cpu can be done at the same time calculations are done (cudaMemcpyAsync)
//by default is running stream 0
...
void setUpCudaDevices()
{
	...
	cudaStreamCreate(&Stream0);                                            //create the stream
	cudaErrorCheck(__FILE__, __LINE__);
...
int main()
{
	...
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS) //1024*1024 (one chunk)  20 times, sinA +cosB = c
	{
		//copy a chunk of A and B from cpu to gpu, have the copying be done asynchronously
