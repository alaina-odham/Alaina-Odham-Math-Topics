...
void setup()
{
	...
	float d, dx, dy, dz;
   	...
   	BlockSize.x = 256;                                       //change this to set amount
	BlockSize.y = 1;
	BlockSize.z = 1;
	
	GridSize.x = ((N-1)/BlockSize.x + 1);                    //change this for mult blocks
	GridSize.y = 1;
	GridSize.z = 1;
...
__global__ void leapFrog(float3 *p, float3 *v, float3 *f, float *m, float g, float h, float damp, float dt, float t, int n)
{
	...
	int i = blockIdx.x * blockDim.x + threadIdx.x;                //change this for mult blocks

	if(i >= n) return;                                            //stop working if threads outnumber sphere number
	...
	//__syncthreads();                                            //remove syncthreads for multiple block use
