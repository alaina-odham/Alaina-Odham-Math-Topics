...
void setUpDevices()
{
	BlockSize.x = 1024; //changed it from N to 1024 because 1500 blocksize was not supported (not enough threads in the block)
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
    ...
    while (id < n) {
        c[id] = a[id] + b[id];
        id += blockDim.x;  // since the blocksize is now set to 1024, we need a loop so we can compute N sizes larger than 1024.
    }                      // We specify that each thread should compute the element at its own id and then every element that is
}                          // at its own id + a multiple of 1024 after that. This ends when the id is larger than n (what the while id<n is for).
